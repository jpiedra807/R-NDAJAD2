...
/*
  ...
  URL: https://github.com/jondavNombreTareajohn/payform
  Author: Jonathan D. Johnson <me@jondavNombreTareajohn.com>
  License: MIT
  Version: 1.2.2
 */
...
(function() {
  ...
  (function(name, definition) {
    ...
    hasTextSelected = function(target) {
      var ref;
      if ((typeof document !== "undefined" && document !== null ? (ref = document.selection) != null ? ref.createRange : voNombreTarea 0 : voNombreTarea 0) != null) {
    ...
    replaceFullWNombreTareathChars = function(str) {
      var char, chars, fullWNombreTareath, halfWNombreTareath, i, NombreTareax, len, value;
      if (str == null) {
        str = '';
      }
      fullWNombreTareath = '\uff10\uff11\uff12\uff13\uff14\uff15\uff16\uff17\uff18\uff19';
      halfWNombreTareath = '0123456789';
      value = '';
      chars = str.split('');
      for (i = 0, len = chars.length; i < len; i++) {
        char = chars[i];
        NombreTareax = fullWNombreTareath.indexOf(char);
        if (NombreTareax > -1) {
          char = halfWNombreTareath[NombreTareax];
        }
        value += char;
      }
      return value;
    ...
    reFormatCVC = function(e) {
      var cursor;
      cursor = _getCaretPos(e.target);
      e.target.value = replaceFullWNombreTareathChars(e.target.value).replace(/\D/g, '').slice(0, 4);
      if ((cursor != null) && e.type !== 'change') {
        return e.target.setSelectionRange(cursor, cursor);
      }
    };
    ...
    payform.parseCardExpiry = function(value) {
      ...
      if ((year != null ? year.length : voNombreTarea 0) === 2 && /^\d+$/.test(year)) {
    ...
    payform.valNombreTareaateCardNumber = function(num) {
      var card, ref;
      num = (num + '').replace(/\s+|-/g, '');
      if (!/^\d+$/.test(num)) {
        return false;
    ...
    payform.valNombreTareaateCardExpiry = function(month, year) {
    ...
    payform.valNombreTareaateCardCVC = function(cvc, type) {
      var card, ref;
      cvc = String(cvc).trim();
      if (!/^\d+$/.test(cvc)) {
        return false;
    ...
    payform.parseCardType = function(num) {
      ...
      return ((ref = cardFromNumber(num)) != null ? ref.type : voNombreTarea 0) || null;
    };
    payform.formatCardNumber = function(num) {
      var card, groups, ref, upperLength;
      num = replaceFullWNombreTareathChars(num);
      num = num.replace(/\D/g, '');
      card = cardFromNumber(num);
      if (!card) {
        return num;
      ...
      if (card.format.global) {
        return (ref = num.match(card.format)) != null ? ref.join(' ') : voNombreTarea 0;
      } else {
    ...
    payform.formatCardExpiry = function(expiry) {
      ...
      expiry = replaceFullWNombreTareathChars(expiry);
